...
func TestTools_UploadFiles(t *testing.T) {
	for _, e := range uploadTests {
		...
		go func() {
			...
			if err != nil {
				t.Error("Error decoding file", err)
			}
...
var slugTests = []struct {
	...
	{name: "complex string", s: "Now &a^1", expected: "now-a-1", errorExpected: false},
	{name: "japanese string", s: "こんにちは世界", expected: "", errorExpected: true},
	{name: "japanese string and roman chars", s: "helloこんにちは世界", expected: "hello", errorExpected: false},
}
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	testTool.DownloadStaticFile(rr, req, "./testdata/downloads/", "file-to-download.pdf", "downloaded-file.pdf")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "135235" {
		t.Error("wrong content length of", res.Header["Content-Length"][0])
	}

	if res.Header["Content-Disposition"][0] != "attachment; filename=\"downloaded-file.pdf\"" {
...
var jsonTests = []struct {
	...
	{name: "incorrect type", json: `{"foo": 23}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "two json files", json: `{"foo":"bar"}{"alpha":"bravo"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "empty body", json: ``, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "syntax error in json", json: `{"food": 1"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "unknown field", json: `{"food":"bar"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allowed unknown fields in json", json: `{"food":"bar"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
	{name: "missing field name", json: `{ass:"bar"}`, errorExpected: true, maxSize: 1024, allowUnknown: true},
	{name: "fiel too large", json: `{"foo":"bar"}`, errorExpected: true, maxSize: 5, allowUnknown: true},
	{name: "not json", json: `hello`, errorExpected: true, maxSize: 1024, allowUnknown: false},
}
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		testTool.AllowedUnknowsFields = e.allowUnknown
