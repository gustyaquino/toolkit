...
type Tools struct {
	...
	AllowedUnknowsFields bool
}

// RandomString return a string of random characters of lenght n, using
// randomString as the source for the string
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				// check to see if filetype is permitted
				allowed := false
				...
				if !allowed {
					var errorString = "the uploaded filetype is not permitted: " + fileType
...
// Slugify is a very simple function to create a slug from a string
...
// ReadJSON tries to read the body of the request and converts from json int a go data variable
...
// WriteJSON takes a status code and arbitrary data and writes JSON to the client
...
// PushJSONToRemote posts arbitrary data to some URL as JSON and returns response, status code and error
